...
void Unfoldplot(){
...
#endif
  const char* Modelnm[4]={"Pythia8","Pythia8 Flat","MadGraph5","Herwig7"};
  ...
  const char* dirname[4]={"PY8","PY8Flat","MG5","HW7"};
...
//----------------------------------------------------
  ...
  const TString Unfolded = "../../Unfolded/"+Date+"/1D/Unfolded_Data_UL2017_PY8_bin.root";  //Unfolded data by Py8
  TFile *Unfoldroot = TFile::Open(Unfolded);

  const TString Unf_root0 = "../../Unfolded/"+Date+"/1D/Unfolded_PY8_bin_PY8_bin.root";     //Py8- by Py8
  const TString Unf_root1 = "../../Unfolded/"+Date+"/1D/Unfolded_PY8_flat_PY8_bin.root";    //Py8Flat - By Py8
  const TString Unf_root2 = "../../Unfolded/"+Date+"/1D/Unfolded_MG5_PY8_bin_PY8_bin.root"; //MG5 by Py8
  const TString Unf_root3 = "../../Unfolded/"+Date+"/1D/Unfolded_HW7_flat_PY8_bin.root";    //HW7 by Py8
  ...
  const TString Unf_dataCT0 = "../../Unfolded/"+Date+"/1D/Unfolded_Data_UL2017_PY8_bin.root";  //data by Py8
  const TString Unf_dataCT1 = "../../Unfolded/"+Date+"/1D/Unfolded_Data_UL2017_PY8_flat.root"; //data by Py8flat
  const TString Unf_dataCT2 = "../../Unfolded/"+Date+"/1D/Unfolded_Data_UL2017_PY8_bin.root";  //data by MG5
  const TString Unf_dataCT3 = "../../Unfolded/"+Date+"/1D/Unfolded_Data_UL2017_HW7_flat.root"; //data by HW7
...
//Reco comparison
  	for(int id=0; id <ndef; id++){
			...
			for(int ik=0; ik<nkappa; ik++){
      				for(int ipt = 0 ; ipt <nHLTmx; ipt++){
			TCanvas *cpt0 = new TCanvas("cpt0", "canvas0", 600,700 );  //for Reco
			...
			//SetMycanvas(cpt0,0,0.1,0.15,0.05,0.1);
			//SetMycanvas(cpt0,0,0.1,0.02,0.05,0.12);
			SetMycanvas(cpt0,0,0.11,0.02,0.05,0.12);
...
//Response Matrix
	for(int imc =0; imc < nmc ; imc++){
  		for(int id=0; id <ndef; id++){
			...
			CTLegend(leg1,Modelnm[imc],Title); leg1->AddEntry((TObject*)0,"" , "");leg1->SetTextColor(1);leg1->Draw();
        		CMS_lumi( cpt0, iPeriod, iPos ); cpt0->Update();
			sprintf(pdfname, "RM_%s_%s.pdf(",dirname[imc],RunEra[iera]); sprintf(pdfname1, "RM_%s_%s.pdf",dirname[imc],RunEra[iera]); sprintf(pdfname2, "RM_%s_%s.pdf)",dirname[imc],RunEra[iera]);
        		if(id==0 && ij==0 && ik==0 && ipt ==0){cpt0->Print(pdfname,"pdf");
        		}else if(id==2 && ij==1 && ik==9 && ipt==9) {cpt0->Print(pdfname2,"pdf");
        		}else{cpt0->Print(pdfname1,"pdf");};
        			}
  	...
  	for(int iun=0; iun < unfold_ty; iun++){
		...
		TCanvas *cpt7 = new TCanvas("cpt5", "canvas5", 600,600);
			...
			SetMycanvas(cpt0,0,0.1,0.02,0.05,0.12);
	  		...
	  		sprintf(pdfname, "TUnfold_%s_%s.pdf(" ,dirname[iun],RunEra[iera]); sprintf(pdfname1, "TUnfold_%s_%s.pdf" ,dirname[iun],RunEra[iera]);sprintf(pdfname2, "TUnfold_%s_%s.pdf)" ,dirname[iun],RunEra[iera]);
	  		if(id==0 && ij==0 && ik==0 && ipt ==0){cpt0->Print(pdfname,"pdf");
                        }else if(id==2 && ij==1 && ik==9 && ipt==9) {cpt0->Print(pdfname2,"pdf");
			...
			//CTLegend(leg1,"Unfolded with Pythia8",Title); leg1->AddEntry((TObject*)0,"" , ""); leg1->AddEntry((TObject*)0,"" , "");leg1->SetTextColor(1);leg1->Draw();
			CTLegend(leg1,"",Title); leg1->AddEntry((TObject*)0,"" , ""); leg1->AddEntry((TObject*)0,"" , "");leg1->SetTextColor(1);leg1->Draw();
       
       			CMS_lumi( cpt5, iPeriod, iPos ); cpt5->Update();       
       			sprintf(pdfname, "TUnfold_corr_%s_%s.pdf(",dirname[iun],RunEra[iera]); sprintf(pdfname1, "TUnfold_corr_%s_%s.pdf" ,dirname[iun],RunEra[iera]);sprintf(pdfname2, "TUnfold_corr_%s_%s.pdf)" ,dirname[iun],RunEra[iera]);
			...
			Prob[id][ij][ik][ipt]->Draw("colz"); leg1->Draw();
       			...
       			sprintf(pdfname, "TUnfold_prob_%s_%s.pdf(" ,dirname[iun],RunEra[iera]); sprintf(pdfname1, "TUnfold_prob_%s_%s.pdf" ,dirname[iun],RunEra[iera]);sprintf(pdfname2, "TUnfold_prob_%s_%s.pdf)" ,dirname[iun],RunEra[iera]);
			...
			Set2dHist(Ematrix[id][ij][ik][ipt],lplot_xtitle, lplot_ytitle,"Covariance",titoff1, titsize1);
       			...
       			sprintf(pdfname, "TUnfold_cov_%s_%s.pdf(" ,dirname[iun],RunEra[iera]); sprintf(pdfname1, "TUnfold_cov_%s_%s.pdf" ,dirname[iun],RunEra[iera]);sprintf(pdfname2, "TUnfold_cov_%s_%s.pdf)" ,dirname[iun],RunEra[iera]);
...
//Closure Test Plot with histogram
        ...
        for(int icl=0; icl < clos_ty ; icl++){
			...
			sprintf(pdfname, "Closure_%s_by_%s_%s.pdf(" ,dirname[icl],dirname[iun],RunEra[iera]); sprintf(pdfname1, "Closure_%s_by_%s_%s.pdf" ,dirname[icl],dirname[iun],RunEra[iera]);sprintf(pdfname2, "Closure_%s_by_%s_%s.pdf)" ,dirname[icl],dirname[iun],RunEra[iera]);
...
//Closure same data with different MC
        for(int id=0; id<ndef; id++){
			...
			float lpos1[8] ={0.45,0.15,0.60,0.35,.055, 1.6,0.2,0.1};
                        ...
                        sprintf(pdfname, "DataCT_%s_%s.pdf(" ,dirname[iun],RunEra[iera]); sprintf(pdfname1, "DataCT_%s_%s.pdf" ,dirname[iun],RunEra[iera]);sprintf(pdfname2, "DataCT_%s_%s.pdf)" ,dirname[iun],RunEra[iera]);
                        if(id==0 && ij==0 && ik==0 && ipt==0){cpt0->Print(pdfname,"pdf");
                        }else if(id==2 && ij==1 && ik==9 && ipt ==9) {cpt0->Print(pdfname2,"pdf");
                        }else{cpt0->Print(pdfname,"pdf");};
                        }
...
//Refold Plot with histogram
        for(int icl=0; icl < clos_ty ; icl++){
			...
			sprintf(pdfname, "Refold_%s_by_%s_%s.pdf(" ,dirname[icl],dirname[iun],RunEra[iera]);sprintf(pdfname1, "Refold_%s_by_%s_%s.pdf" ,dirname[icl],dirname[iun],RunEra[iera]);sprintf(pdfname2, "Refold_%s_by_%s_%s.pdf)" ,dirname[icl],dirname[iun],RunEra[iera]);
                        if(id==0 && ij==0 && ik==0 && ipt==0){cpt0->Print(pdfname,"pdf");
                        }else if(id==2 && ij==1 && ik==9 && ipt ==9) {cpt0->Print(pdfname2,"pdf");
                        }else{cpt0->Print(pdfname,"pdf");};
                                }
...
TCanvas *ratio_can(int Nplot[2],float plegend[7], TH1D* data, TH1D* MC[Nplot[0]], char* lowpadx, const char* modnam[Nplot[0]], const  char* datanm[1]){
	...
	//TPad *padfun1 = new TPad("padfun1", "padfun1", 0, 0.02,2.0, 0.30);
	...
	//TPad *padfun2 = new TPad("padfun2", "padfun2", 0, 0.02,2.0, 0.30);
