...
//----------------------------------------------------
//Unfolded data with different MC
	for(int icl=0; icl < clos_ty; icl++){
  		for(int iun=0; iun < unfold_ty; iun++){
    			for(int id=0; id<ndef; id++){
                                for(int ij=0; ij<njet; ij++){
  		...
  		for(int id=0; id<ndef; id++){
			...
			divBybinWidth(MC_input[0]);
          		MCinput_index[0] = Recomcname[icl];
       			...
       			//sprintf(pdfname, "%i_%sRefoldHist_%s_%s.pdf(" ,icl,"dd_",RunEra[iera],dirname[iun]); sprintf(pdfname1, "%i_%sRefoldHist_%s_%s.pdf" ,icl,"dd_",RunEra[iera],dirname[iun]);sprintf(pdfname2, "%i_%sRefoldHist_%s_%s.pdf)" ,icl,"dd_",RunEra[iera],dirname[iun]);
