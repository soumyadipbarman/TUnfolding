...
void testUnfold2c()
{
  ...
  TFile *inputData=new TFile("Test_MC_2017UL.root");
  TFile *inputMC=new TFile("Test_MC_2017UL.root");
  
  //TFile *inputMC1=new TFile("MG_UL17_binned_6July20.root");
  TFile *inputMC1=new TFile("Test_MC_2017UL.root");
  //TFile *inputMC1=new TFile("PY8_UL17_ULbinned_1July20.root");
  TFile *inputMC2=new TFile("Test_MC_2017UL.root");
  ...
  */
  ...
  TH1D *PsudoData_Gen[ndef][njet][nkappa][njetptmn];    //Gen Level Psudo Data
  TH2D *h2dGenDetMC[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco
  ...
  TH2D *h2dGenDetMC1[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco copy
  ...
  TH2D *h2dGenDetMC2[ndef][njet][nkappa][njetptmn];  //// MC generator Vs Reco copy
 
  TH1D *MC_Reco_re[ndef][njet][nkappa][njetptmn];  //Reco MC Rebins If needed 
  TH1D *MC_Gen_re[ndef][njet][nkappa][njetptmn];   //Generator Rebins If Needed
  TH1D *Data_Reco_re[ndef][njet][nkappa][njetptmn];    //Reconst Data with Rebin
  TH2D *h2dGenDetMC_re[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco Rebin
 
  //---------------------MG5
  TH1D *MG5_MC_Reco[ndef][njet][nkappa][njetptmn];  //Reconstructed MC
  TH1D *MG5_MC_Gen[ndef][njet][nkappa][njetptmn];   //Generator level MC
  TH2D *MG5_h2dGenDetMC[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco
  ...
  TH2D *MG5_h2dGenDetMC1[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco copy
  ...
  TH2D *MG5_h2dGenDetMC2[ndef][njet][nkappa][njetptmn];
 
  TH1D *MG5_MC_Reco_re[ndef][njet][nkappa][njetptmn];  //Reconstructed MC
  TH1D *MG5_MC_Gen_re[ndef][njet][nkappa][njetptmn];   //Generator level MC
  TH2D *MG5_h2dGenDetMC_re[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco
  //----------------------HW
  TH1D *HW7_MC_Reco[ndef][njet][nkappa][njetptmn];  //Reconstructed MC
  TH1D *HW7_MC_Gen[ndef][njet][nkappa][njetptmn];   //Generator level MC
  TH2D *HW7_h2dGenDetMC[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco
  ...
  TH2D *HW7_h2dGenDetMC1[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco copy
  ...
  TH2D *HW7_h2dGenDetMC2[ndef][njet][nkappa][njetptmn];
 
  TH1D *HW7_MC_Reco_re[ndef][njet][nkappa][njetptmn];  //Reconstructed MC
  TH1D *HW7_MC_Gen_re[ndef][njet][nkappa][njetptmn];   //Generator level MC
  TH2D *HW7_h2dGenDetMC_re[ndef][njet][nkappa][njetptmn];   // MC generator Vs Reco
 ...
 */
  ...
  TH1D* rebin1d_hist(TH1D* thin, int idef, int ijet, int ikappa, int ijetpt, int arrayvar_first[3][2][10][10],int arrayvar_last[3][2][10][10]);
  TH2D* rebin2d_hist(TH2D* thin, TH1D* MC_reco,TH1D* MC_gen, int idef, int ijet, int ikappa, int ijetpt, int arrayvar_first[3][2][10][10],int arrayvar_last[3][2][10][10],int arrayvar_firstG[3][2][10][10],int arrayvar_lastG[3][2][10][10] );
  //TH1D* rebin1d_hist_gen(TH1D* thin, int itype, int ijetpt, int ivar );
  TH1D* rebin1d_hist_gen(TH1D* thin, int idef, int ijet, int ikappa, int ijetpt, int arrayvar_first[3][2][10][10],int arrayvar_last[3][2][10][10]);
 	...
 	for (int ij=1; ij<3; ij++){
		for (int ik=0; ik<10; ik++){
			for(int ipt=0; ipt< njetptmn; ipt++){
       		//Reco Data
       	 	inputDir0->cd();
       		sprintf(histname, "analyzeBasicPat/reco_jc_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //reco_jc_D1_j1_k0_pt0_eta0
		//cout << histname << endl;
...
#ifdef CLOUSER
       sprintf(histname, "analyzeBasicPat/gen_jc_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //gen_jc_D1_j1_k0_pt0_eta0
       ...
       //MC Fake
       sprintf(histname, "analyzeBasicPat/recofake_jc_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //reco_typ_1_pt4_eta0_24 recofake_jc_D1_j1_k0_pt0_eta0
       MC_fake1[id][ij][ik][ipt] = (TH1D*) inputMC->Get(histname);
       
       //cout << " Fake= " <<MC_fake1[id][ij][ik][ipt]->GetEntries() <<" Reco-fake: " <<(MC_Reco1[id][ij][ik][ipt]->GetEntries() - MC_fake1[id][ij][ik][ipt]->GetEntries());
       ...
       //MC miss 
       sprintf(histname, "analyzeBasicPat/genmiss_jc_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //reco_typ_1_pt4_eta0_24 genmiss_jc_D1_j1_k0_pt0_eta0
       MC_miss1[id][ij][ik][ipt] = (TH1D*) inputMC->Get(histname);
       //cout << "Testing 1" <<histname<<endl;
       ...
       //Response Matrix
       sprintf(histname, "analyzeBasicPat/RM_jc_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //corr_typ_0_pt2_eta0_3 RM_jc_D1_j1_k0_pt0_eta0    
       h2dGenDetMC1[id][ij][ik][ipt] = (TH2D*) inputMC->Get(histname);   //Xgen(coarse) , Yreco(fine)
...
//A kind of way to remove underflow and overlow bins
       ...
       TH2D *NewRecoGen;
       NewData = (TH1D*)Data_Reco1[id][ij][ik][ipt]->Clone();
       NewReco = (TH1D*)MC_Reco1[id][ij][ik][ipt]->Clone();
       NewGen  = (TH1D*)MC_Gen1[id][ij][ik][ipt]->Clone();
       NewRecoGen = (TH2D*)h2dGenDetMC1[id][ij][ik][ipt]->Clone();
       
       Newmiss  = (TH1D*)MC_miss1[id][ij][ik][ipt]->Clone();
       Newfake = (TH1D*)MC_fake1[id][ij][ik][ipt]->Clone();
	 ...
	 NewData->SetBinContent(ix,Data_Reco1[id][ij][ik][ipt]->GetBinContent(ix));
	 NewData->SetBinError(ix, sqrt(Data_Reco1[id][ij][ik][ipt]->GetBinError(ix)* Data_Reco1[id][ij][ik][ipt]->GetBinError(ix)));
       }
       
       for(int ix=1; ix < NbinxR+1 ; ix++){
	 NewReco->SetBinContent(ix,MC_Reco1[id][ij][ik][ipt]->GetBinContent(ix));
	 NewReco->SetBinError(ix, sqrt(MC_Reco1[id][ij][ik][ipt]->GetBinError(ix)* MC_Reco1[id][ij][ik][ipt]->GetBinError(ix)));
       }
       for(int ix=1; ix < NbinxG+1 ; ix++){
	 NewGen->SetBinContent(ix,MC_Gen1[id][ij][ik][ipt]->GetBinContent(ix));
	 NewGen->SetBinError(ix, sqrt(MC_Gen1[id][ij][ik][ipt]->GetBinError(ix)* MC_Gen1[id][ij][ik][ipt]->GetBinError(ix)));
       }       
       for(int ix=1; ix < NbinMx+1 ; ix++){
	 for(int iy=1; iy < NbinMy+1 ; iy++){
	   NewRecoGen->SetBinContent(ix, iy, h2dGenDetMC1[id][ij][ik][ipt]->GetBinContent(ix,iy));
	   NewRecoGen->SetBinError(ix, iy, sqrt(h2dGenDetMC1[id][ij][ik][ipt]->GetBinError(ix,iy)* h2dGenDetMC1[id][ij][ik][ipt]->GetBinError(ix,iy)));
	 }
       }
       //NewRecoGen->SetBinContent(0, 0,0.0); NewRecoGen->SetBinError(0,0,0.0);
       for(int ix=1; ix < NbinxR+1 ; ix++){
	 Newfake->SetBinContent(ix,MC_fake1[id][ij][ik][ipt]->GetBinContent(ix));
	 Newfake->SetBinError(ix, sqrt(MC_fake1[id][ij][ik][ipt]->GetBinError(ix)* MC_fake1[id][ij][ik][ipt]->GetBinError(ix)));
       }
       for(int ix=1; ix < NbinxG+1 ; ix++){
	 Newmiss->SetBinContent(ix,MC_miss1[id][ij][ik][ipt]->GetBinContent(ix));
	 Newmiss->SetBinError(ix, sqrt(MC_miss1[id][ij][ik][ipt]->GetBinError(ix)* MC_miss1[id][ij][ik][ipt]->GetBinError(ix)));
       }
     ...
     //    NewRecoGen->SetBinContent(0, iy, Newmiss->GetBinContent(iy)+ h2dGenDetMC1[ity][ivar][ipt]->GetBinContent(0,iy));
       ...
       Data_Reco1[id][ij][ik][ipt] = (TH1D*)NewData->Clone();
       MC_Reco1[id][ij][ik][ipt] = (TH1D*)NewReco->Clone();
       MC_Gen1[id][ij][ik][ipt]  = (TH1D*)NewGen->Clone();
       h2dGenDetMC1[id][ij][ik][ipt] = (TH2D*)NewRecoGen->Clone();
       
       MC_fake1[id][ij][ik][ipt]  = (TH1D*)Newfake->Clone();
       MC_miss1[id][ij][ik][ipt]  = (TH1D*)Newmiss->Clone();
...
//--------------------------------Fake and Miss rate : Thanks to Patrick  14Aug20
       ...
       dataold = Data_Reco1[id][ij][ik][ipt]->GetName();
       sprintf(datanew, "Data_%s", dataold);  Data_Reco1[id][ij][ik][ipt]->SetName(datanew);
       
       Data_Reco_re[id][ij][ik][ipt] = rebin1d_hist(Data_Reco1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first, arrayvar_last);  // change arrayvar_first
       
       MC_Reco_re[id][ij][ik][ipt] = rebin1d_hist(MC_Reco1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last);
       ...
       MC_Gen_re[id][ij][ik][ipt] = (TH1D*)MC_Gen1[id][ij][ik][ipt]->Clone();
       cout << " Rebin MC Gen bin : " <<  MC_Gen_re[id][ij][ik][ipt]->GetNbinsX() << endl;
       
       
       h2dGenDetMC_re[id][ij][ik][ipt] = rebin2d_hist(h2dGenDetMC1[id][ij][ik][ipt], MC_Reco1[id][ij][ik][ipt], MC_Gen1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last,arrayvar_firstG,arrayvar_lastG);
       cout << "ReBin REco Bin :"<< h2dGenDetMC_re[id][ij][ik][ipt]->GetNbinsX() <<" Rebin Gen Bin : " << h2dGenDetMC_re[id][ij][ik][ipt]->GetNbinsY()<< endl;
       
       //Define data MC to be used for unfolding
         Data_Reco[id][ij][ik][ipt] = (TH1D*)Data_Reco_re[id][ij][ik][ipt]->Clone();
         MC_Reco[id][ij][ik][ipt] = (TH1D*)MC_Reco_re[id][ij][ik][ipt]->Clone();
         MC_Gen[id][ij][ik][ipt] = (TH1D*)MC_Gen_re[id][ij][ik][ipt]->Clone();
         h2dGenDetMC[id][ij][ik][ipt] = (TH2D*)h2dGenDetMC_re[id][ij][ik][ipt]->Clone();
...
//------------------Read Madgraph
  for(int id=1; id <4; id++){
...
#ifdef REBIN
       MG5_MC_Reco_re[id][ij][ik][ipt] = rebin1d_hist(MG5_MC_Reco1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last);
       //MG5_MC_Reco_re[ity][ivar][ipt]->Rebin(4);
	 
       //MG5_MC_Gen_re[ity][ivar][ipt] = rebin1d_hist(MG5_MC_Gen1[ity][ivar][ipt], ity, ipt, ivar);
       MG5_MC_Gen_re[id][ij][ik][ipt] = rebin1d_hist_gen(MG5_MC_Gen1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last);
       //MG5_MC_Gen_re[ity][ivar][ipt]->Rebin(2);
       //cout << " Rebin MC Gen bin : " <<  MG5_MC_Gen_re[ity][ivar][ipt]->GetNbinsX() << endl;
	 
       MG5_h2dGenDetMC_re[id][ij][ik][ipt] = rebin2d_hist(MG5_h2dGenDetMC1[id][ij][ik][ipt], MG5_MC_Reco1[id][ij][ik][ipt],MG5_MC_Gen1[id][ij][ik][ipt] ,id, ij, ik, ipt, arrayvar_first,arrayvar_last,arrayvar_firstG,arrayvar_lastG);
       ...
       cout << "ReBin REco Bin :"<< MG5_h2dGenDetMC_re[id][ij][ik][ipt]->GetNbinsX() <<" Rebin Gen Bin : " << MG5_h2dGenDetMC_re[id][ij][ik][ipt]->GetNbinsY()<< endl;
       //Define data MC to be used for unfolding
       MG5_MC_Reco[id][ij][ik][ipt] = (TH1D*)MG5_MC_Reco_re[id][ij][ik][ipt]->Clone();
       MG5_MC_Gen[id][ij][ik][ipt] = (TH1D*)MG5_MC_Gen_re[id][ij][ik][ipt]->Clone();
       MG5_h2dGenDetMC[id][ij][ik][ipt] = (TH2D*)MG5_h2dGenDetMC_re[id][ij][ik][ipt]->Clone();
       cout << "Histogram MG Read and Rebin OK" <<endl;
#else
       MG5_MC_Reco[id][ij][ik][ipt] = (TH1D*)MG5_MC_Reco1[id][ij][ik][ipt]->Clone();
       MG5_MC_Gen[id][ij][ik][ipt] = (TH1D*)MG5_MC_Gen1[id][ij][ik][ipt]->Clone();
       MG5_h2dGenDetMC[id][ij][ik][ipt] = (TH2D*)MG5_h2dGenDetMC1[id][ij][ik][ipt]->Clone();
#endif
       inputRebin2->cd();
       MG5_MC_Reco[id][ij][ik][ipt]->Write();
       MG5_MC_Gen[id][ij][ik][ipt]->Write();
       MG5_h2dGenDetMC[id][ij][ik][ipt]->Write();
	}
     }
   }
 }
 ...
 //-----------HW7
   for(int id=1; id <4; id++){
...
#ifdef REBIN
        HW7_MC_Reco_re[id][ij][ik][ipt] = rebin1d_hist(HW7_MC_Reco1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last);
	// HW7_MC_Reco_re[ity][ivar][ipt]->Rebin(4);
	
	//  HW7_MC_Gen_re[ity][ivar][ipt] = rebin1d_hist(HW7_MC_Gen1[ity][ivar][ipt], ity, ipt, ivar);
        HW7_MC_Gen_re[id][ij][ik][ipt] = rebin1d_hist_gen(HW7_MC_Gen1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last);
	//HW7_MC_Gen_re[ity][ivar][ipt]->Rebin(2);
        //cout << " Rebin MC Gen bin : " <<  HW7_MC_Gen_re[ity][ivar][ipt]->GetNbinsX() << endl;
	
        HW7_h2dGenDetMC_re[id][ij][ik][ipt] = rebin2d_hist(HW7_h2dGenDetMC1[id][ij][ik][ipt], HW7_MC_Reco1[id][ij][ik][ipt], HW7_MC_Gen1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first,arrayvar_last,arrayvar_firstG,arrayvar_lastG);
	...
	//cout << "ReBin REco Bin :"<< HW7_h2dGenDetMC_re[ity][ivar][ipt]->GetNbinsX() <<" Rebin Gen Bin : " << HW7_h2dGenDetMC_re[ity][ivar][ipt]->GetNbinsY()<< endl;
        ...
        //Define data MC to be used for unfolding
	HW7_MC_Reco[id][ij][ik][ipt] = (TH1D*)HW7_MC_Reco_re[id][ij][ik][ipt]->Clone();
	HW7_MC_Gen[id][ij][ik][ipt] = (TH1D*)HW7_MC_Gen_re[id][ij][ik][ipt]->Clone();
	HW7_h2dGenDetMC[id][ij][ik][ipt] = (TH2D*)HW7_h2dGenDetMC_re[id][ij][ik][ipt]->Clone();
	
	
        cout << "Histogram HW Read and Rebin OK" <<endl;
#else
        HW7_MC_Reco[id][ij][ik][ipt] = (TH1D*)HW7_MC_Reco1[id][ij][ik][ipt]->Clone();
        HW7_MC_Gen[id][ij][ik][ipt] = (TH1D*)HW7_MC_Gen1[id][ij][ik][ipt]->Clone();
        HW7_h2dGenDetMC[id][ij][ik][ipt] = (TH2D*)HW7_h2dGenDetMC1[id][ij][ik][ipt]->Clone();
	 
#endif
        inputRebin3->cd();
        HW7_MC_Reco[id][ij][ik][ipt]->Write();
        HW7_MC_Gen[id][ij][ik][ipt]->Write();
	HW7_h2dGenDetMC[id][ij][ik][ipt]->Write();
	}
     }
   }
 }
 ...
 //cross check efficincy and purity calculation (Tunfold example code)
    for(int id=1; id <4; id++){
       ...
       for(int binGen=0;binGen<= h2dGenDetMC[id][ij][ik][ipt]->GetNbinsY()+1;binGen++) {
         double sum0=0.;
         double sum1=0.;
         for(int binRec=0;binRec<= h2dGenDetMC[id][ij][ik][ipt]->GetNbinsX()+1;
             binRec++) {
...
//int bias[5]={2.0,2.0,2.0,2.0,2.0};
    ...
    for(int id=1; id <4; id++){
	 ...
	 (hist_migrationCoarseFine_MC,TUnfoldDensity::kHistMapOutputVert,regMode, constraintMode,densityFlags,0,0,REGULARISATION_DISTRIBUTION,REGULARISATION_AXISSTEERING);//,binningCoarseGen, binningFineReco);
       ...
       sprintf(unfoldhist, "Tunfold_lscan_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt);
       sprintf(title, "Tunfolded Tikhonov Lcurve %i 2.5 %s %s %s", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       sprintf(Rhoname, "Tunfold_lscan_1dcorr_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt);
       sprintf(Rhotitle, "correlation coefficients Tikhonov Lcurve %i 2.5 %s %s %s ", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       sprintf(Rhoname2d, "Tunfold_lscan_corr_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //unfolded_typ_0_pt2_eta0_3
       sprintf(Rhotitle2d, "2D correlation coefficients Tikhonov Lcurve %i 2.5 %s %s %s ", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       sprintf(Ematrix, "Tunfold_lscan_Emat_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //unfolded_typ_0_pt2_eta0_3
       sprintf(Ematrixtitle, "covariance matrix all contributions Tikhonov Lcurve %i 2.5 %s %s %s", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       sprintf(probMat, "Tunfold_lscan_probM_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt);
       sprintf(probmat_title, "Probability matrix Tikhonov Lcurve %i 2.5 %s %s %s", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       sprintf(foldback, "Tunfold_lscan_Refold_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt); //unfolded_typ_0_pt2_eta0_3
       sprintf(foldback_title, "Folded back  Lcurve %i 2.5 %s %s %s", int(leadingPtThreshold[ipt]), obs_def[id], jet_num[ij], k_fact[ik]);
       ...
       for (int i = 1; i <= hist_PTunfolded_TikhonovLCurve->GetNbinsX(); ++i) {
         ...
         double factor = 1;
         factor += MC_miss[id][ij][ik][ipt]->GetBinContent(i);
         content *= factor;
       ...
       sprintf(Lcursure, "Tunfold_lscan_Lcurve_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt);
       sprintf(TauLsure, "Tau_TikhLscan_D%i_j%i_k%i_pt%i_eta0", id, ij, ik, ipt);
...
TH1D* rebin1d_hist(TH1D* thin, int idef, int ijet, int ikappa , int ijetpt, int arrayvar_first[3][2][10][10],int arrayvar_last[3][2][10][10]){
  ...
  if (nxmod2<0) {
    for (int ij=0; ij<nbinx+2; ij++) {
      if (ij <=arrayvar_first[idef][ijet][ikappa][ijetpt]) {
      } else if ( ij > (nbinx - arrayvar_last[idef][ijet][ikappa][ijetpt])) {
      } else {
...
//Get Gen Level entries
  if (ngen<0) {
    for (int ij=0; ij<nbiny+2; ij++) {
      if (ij <=arrayvar_firstG[idef][ijet][ikappa][ijetpt]) {
      } else if ( ij > (nbiny - arrayvar_lastG[idef][ijet][ikappa][ijetpt])) {
      } else {
  ...
  if (xfirst==0 && yfirst==0) {
    for (int iy=0; iy<nbiny+2; iy++) {
      if (iy <=arrayvar_firstG[idef][ijet][ikappa][ijetpt]) {
      } else if ( iy > (nbiny - arrayvar_lastG[idef][ijet][ikappa][ijetpt]) ) {
      } else {
	xfirst=0;
      for (int ix=0; ix<nbinx+2; ix++) {
        if (ix <=arrayvar_first[idef][ijet][ikappa][ijetpt]) {
      } else if (ix > (nbinx - arrayvar_last[idef][ijet][ikappa][ijetpt])) {
      } else {
...
//1D Rebin function:Basically it cut off some bin you  don't needed
TH1D* rebin1d_hist_gen(TH1D* thin, int idef, int ijet, int ikappa, int ijetpt, int arrayvar_firstG[3][2][10][10],int arrayvar_lastG[3][2][10][10]){
TH1D* thout;
int nxmod2 = -1; double xmod2[300];
...
char namey[100], titley[100];
  ...
  if (nxmod2<0) {
    for (int ij=0; ij<nbinx+2; ij++) {
      if (ij <=arrayvar_firstG[idef][ijet][ikappa][ijetpt]) {
      } else if ( ij > (nbinx - arrayvar_lastG[idef][ijet][ikappa][ijetpt])) {
      } else { 
        xmod2[ifirst] = thin->GetBinLowEdge(ij);
        xmod2[ifirst+1] = thin->GetBinLowEdge(ij+1);
        yvl[ifirst] =thin->GetBinContent(ij);
