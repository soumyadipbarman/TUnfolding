...
void testUnfold2c()
{
  ...
  TFile *inputMC1=new TFile("PY_CP5_QCD_Pt-15to7000_TuneCP5_Flat_13TeV_pythia8.root");
		...
		for (int ik=0; ik<nkappa; ik++){
			for(int ipt=0; ipt<njetptmn; ipt++){
      		...
      		Data_Reco1[id][ij][ik][ipt] = (TH1D*) inputData->Get(histname);
       		...
       		for (int ibin =1 ; ibin <  Data_Reco1[id][ij][ik][ipt]->GetNbinsX()+1; ibin++ ){ // why added +1 ?
...
//--------------------------------Fake and Miss rate : Thanks to Patrick  14Aug20
		...
		Data_Reco_re[id][ij][ik][ipt] = rebin1d_hist(Data_Reco1[id][ij][ik][ipt], id, ij, ik, ipt, arrayvar_first, arrayvar_last);
         	...
         	cout << "Rebin for Pythia8 Done"<<endl;
#else
...
double Condition (TH2 * RM)
{
    ...
    TH1 * RMx = RM->ProjectionY("RMx", 0, -1);

    // normalisation & condition
    TMatrixD m(Nx,Ny); // unfortunately, we have to swap the axes...
    for (int i = 1; i <= Ny; ++i) {
