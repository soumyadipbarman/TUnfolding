...
void Unfoldplot(){
  ...
  static const int clos_ty =2;   //Number of Closure Test performed
  static const int nHLTmx=10;    //PT Range
  static const int nmc = 2;      //Number of MC sample
  ...
  static const int iera = 1;  // which run year is used
  int iPeriod = 0;  
  int iPos=0 ;

  const char* dirname[4]={"PY8","PY8Flat","MG5","HW7"};
  const char* mcnamerco[4]={"Pythia8","PY8 Flat","MadGraph5","Herwig7"};
...
//----------------------------------------------------
  ...
  const TString Unfolded = "../../Unfolded/"+Date+"/2D/Unfolded_PY8_bin_PY8_bin.root";  //Unfolded data by Py8
  ...
  //const TString Unf_dataCT0 = "../../Unfolded/"+Date+"/2D/Unfolded_PY8_bin_PY8_bin.root";  //data by Py8
  //const TString Unf_dataCT1 = "../../Unfolded/"+Date+"/2D/Unfolded_PY8_bin_PY8_bin.root"; //data by Py8flat
...
//----------------------------------------------------
  ...
  TH1D *MC_genM[nmc][ndef][njet][nkappa];    //MC gen projectionx
  TH2D *MC_gen2d[nmc][ndef][njet][nkappa];   //MC gen true bin
  TH1D *MC_genmiss[nmc][ndef][njet][nkappa]; //MC Gen-miss

  TH1D *MC_reco[nmc][ndef][njet][nkappa];    //MC Reco
  TH1D *MC_recoM[nmc][ndef][njet][nkappa];    //MC gen projectionx
  TH2D *MC_reco2d[nmc][ndef][njet][nkappa];  //MC Reco true bin
  TH1D *MC_recofake[nmc][ndef][njet][nkappa];//MC Reco -Fake
  
  TH2D *MC_Res[nmc][ndef][njet][nkappa];     //RM

  TH1D *hist_eff[nmc][ndef][njet][nkappa];   //efficiency (inverse of missrate)
  TH1D *hist_fake[nmc][ndef][njet][nkappa];  //fakerate
  TH1D *hist_purity[nmc][ndef][njet][nkappa];//purity
  TH1D *hist_stbl[nmc][ndef][njet][nkappa];  //stability
...
//Read MC
  	for(int imc =0; imc < nmc ; imc++){
		...
		MC_recoM[imc][id][ij][ik] = (TH1D*)ReadHist1D(mcdir[imc]+"/Edd_reco_d"+to_string(id)+"_j"+to_string(ij)+"_k"+ to_string(ik)+"_eta0_px",Unfoldroot);
...
}//end of MCinput root file  reading
//----------------------------------------------------
//Read Unfolded MC for CT
	...
	for(int icl=0; icl < clos_ty; icl++){
		...
		EmatrixCT[icl][iun][id][ij][ik] = (TH2D*)ReadHist2D(unfdir+"/dd_Emat_NoReg_d"+to_string(id)+"_j"+to_string(ij)+"_k"+ to_string(ik)+"_eta0",Unf_root[icl]);
...
//Read Unfolded data with PY8
	for(int id=0; id<ndef; id++){
		...
		Ematrix[id][ij][ik] = (TH2D*)ReadHist2D(unfdir+"/dd_Emat_NoReg_d"+to_string(id)+"_j"+to_string(ij)+"_k"+ to_string(ik)+"_eta0",Unfoldroot);
...
for (int iun=0; iun< nmc; iun++){
  	for(int id=0; id<ndef; id++){
		...
		TH1D *MyHist  = (TH1D*)MC_recoHT[0][id][ij][ik][ipt]->Clone();
		Integralhist(MyHist);	
		...
		TH1D *MC_input[1];
		const char *MCinput_index[1];
		const char *data_index[1];
	  	MC_input[0] = (TH1D*) MC_recoHT[1][id][ij][ik][ipt]->Clone();
		Integralhist(MC_input[0]);	 
		//divBybinWidth(MC_input[iout]);
	  	MCinput_index[0] = "PY8Flat"; 

		data_index[0]= "Pythia8";
...
//Response Matrix
	for(int  imc =0; imc < nmc ; imc++){
		...
		CTLegend(leg1,Modelnm[imc],Title); leg1->AddEntry((TObject*)0,obs_def[id] , "");leg1->SetTextColor(-8);
        	CMS_lumi( cpt0, iPeriod, iPos ); cpt0->Update();
...
//Efficiency, Purity, Fake rate, Stability
  	for(int id=0; id<ndef; id++){
        	for(int ij=0; ij<njet; ij++){
                	for(int ik =0 ; ik<nkappa ; ik++){
        	TCanvas *cpt1 = new TCanvas("cpt1", "canvas1", 600,700 ); 
        	TCanvas *cpt2 = new TCanvas("cpt2", "canvas2", 600,700 );  
        	TCanvas *cpt3 = new TCanvas("cpt3", "canvas3", 600,700 );  
        	TCanvas *cpt4 = new TCanvas("cpt4", "canvas4", 600,700 );  

        	SetMycanvas(cpt1,0,0.11,0.03,0.05,0.12);
		...
		char lplot_xtitle[100];
			...
			sprintf(lplot_xtitle,"%s_{%s}^{%s}", obs_def[id], njets[ij], k_fact[ik]);
			Myplotset(hist_effHT[umc][id][ij][ik][ipt], lplot_xtitle ,"Efficiency");
        		hist_effHT[umc][id][ij][ik][ipt]->SetLineColor(color[ipt]);
			hist_effHT[umc][id][ij][ik][ipt]->Draw("same hist "); 
        		leg2->AddEntry(hist_effHT[umc][id][ij][ik][ipt], Title ,"lp");
			if(ipt==9){leg2->Draw();}

        		cpt2->cd();
 			Myplotset(hist_purityHT[umc][id][ij][ik][ipt],lplot_xtitle,"Purity");
 			hist_purityHT[umc][id][ij][ik][ipt]->SetLineColor(color[ipt]);
        		hist_purityHT[umc][id][ij][ik][ipt]->Draw("same hist");
        		if(ipt==9){leg2->Draw();}
			cpt2->Update();

        		cpt3->cd();
        		Myplotset(hist_fakeHT[umc][id][ij][ik][ipt],lplot_xtitle,"Fake rate");
        		hist_fakeHT[umc][id][ij][ik][ipt]->SetLineColor(color[ipt]);
        		hist_fakeHT[umc][id][ij][ik][ipt]->Draw("same hist");
        		if(ipt==9){leg2->Draw();}
       			cpt3->Update();

			cpt4->cd();
        		Myplotset(hist_stblHT[umc][id][ij][ik][ipt],lplot_xtitle,"Stability");
        		hist_stblHT[umc][id][ij][ik][ipt]->SetLineColor(color[ipt]);
        		hist_stblHT[umc][id][ij][ik][ipt]->Draw("same hist");
        		if(ipt==9){leg2->Draw();}
			cpt4->Update();
        	}
...
cout << "Efficiency, Stabilty, Fakerate, Purity " <<endl;
//----------------------------------------------------
//Probability, Correlation, Covariance Matrix Py8
	for(int iun=0; iun < unfold_ty; iun++){
  		for(int id=0; id<ndef; id++){
                	for(int ij=0; ij<njet; ij++){
                       		for(int ik =0 ; ik<nkappa ; ik++){
	 	...
	 	TCanvas *cpt7 = new TCanvas("cpt7", "canvas7", 600,600 );  //for
       		...
       		cpt5->cd();
		sprintf(lplot_ytitle, "Reco bin ID (%s_{%s}^{%s}, P_{T})",obs_def[id], njets[ij], k_fact[ik]);
                sprintf(lplot_xtitle, "Gen bin ID (%s_{%s}^{%s}, P_{T})",obs_def[id], njets[ij], k_fact[ik]);
       		double titoff1[3]={1.2,1.3,1.0};
       		double titsize1[3] ={0.035,0.035,0.035};
		...
		CTLegend(leg1,"",""); leg1->AddEntry((TObject*)0,"" , ""); leg1->AddEntry((TObject*)0,"" , "");leg1->SetTextColor(1);leg1->Draw();
       		...
       		Set2dHist(Prob[id][ij][ik],lplot_xtitle, lplot_ytitle,"Probability",titoff1, titsize1);
       		...
       		Set2dHist(Ematrix[id][ij][ik],lplot_xtitle, lplot_ytitle,"",titoff1, titsize1);
