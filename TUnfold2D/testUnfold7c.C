...
#define READ(TYPE,binning,name)                       \
  ...
  TH1 *histOutputCSURE[3];
  //TH2 *histRhoCLCurve;
  TH2 *histProbC;
#ifdef AUTOMATIC_SCAN
  double tauMin=0.;
  double tauMax=0.;
#else
  double tauMin=1.e-4;
  ...
  tunfoldC->ScanSURE(50,tauMin,tauMax,0);
  histOutputCSURE[0]=tunfoldC->GetOutput("histOutputCSURE");
  CreateHistogramCopies(histOutputCSURE,coarseBinning);

  }
  TH1 *histOutputFtau0[3];
  TH2 *histRhoFtau0;
  TH1 *histOutputFLCurve[3];
  TH1 *histOutputFSURE[3];
  //TH2 *histRhoFLCurve;
  TH2 *histProbF;
  TGraph *lCurve;
  TSpline *logTauX,*logTauY;
#ifndef AUTOMATIC_SCAN
  tauMin=3.E-4;
  tauMax=3.E-2;
#endif
  //double tauF;
  ...
  tunfoldF->ScanSURE(50,tauMin,tauMax);
  histOutputFSURE[0]=tunfoldF->GetOutput("histOutputFSURE");
  CreateHistogramCopies(histOutputFSURE,coarseBinning);
  }
  TH1 *histOutputFAtau0[3];
  TH2 *histRhoFAtau0;
  TH1 *histOutputFALCurve[3];
  TH1 *histOutputFASURE[3];
  TH2 *histRhoFALCurve;
  TH2 *histRhoFASURE;
  ...
  double tauFA,tauFArho,tauFASURE;
  {
  ...
  //tauMin=0.;
  //tauMax=0.;
  tunfoldFA->ScanSURE(50,tauMin,tauMax,&logTauSURE);
  tauFASURE=tunfoldFA->GetTau();
  histOutputFASURE[0]=tunfoldFA->GetOutput("histOutputFASURE");
  histRhoFASURE=tunfoldFA->GetRhoIJtotal("histRhoFASURE");
  CreateHistogramCopies(histOutputFASURE,coarseBinning);

  }
  lCurve->Write();
  logTauX->Write();
  logTauY->Write();
  logTauSURE->Write("logTauSure");
  ...
  TH1 *histOutputFASUREO=AddOverflowX(histOutputFASURE[2],widthC);
  TH2 *histRhoFASUREO=AddOverflowXY(histRhoFASURE,widthC,widthC);
...
#define NREPLICA 300
  ...
  int iFitFASURE=table.size();
  DoFit(histOutputFASUREO,histRhoFASUREO,histDataGenO,"Tikh+area=SURE",table);
  //=========================== template+area+tikhonov =====================
  subc[0]->cd();
  DrawPadTruth(histMcsigGenO,histDataGenO,histOutputFASUREO,"SURE",tauFASURE,
                &table[table.size()-1].second);
  subc[1]->cd();
  DrawPadCorrelations(histRhoFASUREO,&table);
  subc[2]->cd();
  DrawPadReco(histMcRecFO,histMcbgrRecFO,histDataRecFO,
              histOutputFASUREO,histProbFO,histRhoFASUREO);
  c3c->SaveAs("SURE_FA.eps");
  ...
  rhoScan->Draw();
  c4->cd(3);
  logTauSURE->SetTitle(";log_{10}(#tau);SURE");
  logTauSURE->SetLineColor(kRed);
  logTauSURE->Draw("AL");
  //DrawPadTruth(histMcsigGenO,histDataGenO,histOutputFALCurveO,"Tikhonov",tauFA,
